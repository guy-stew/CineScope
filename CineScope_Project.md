...
**Last updated:** 10 March 2026 (Phase 3 planning — political data complete, TMDB done, demographics in progress)
...
| **Council Politics** | council_politics.json — 961 venues, council name, party, colour, control type | ✅ **COMPLETE — in public/data/** |
...
## 6. React App — v2.0.2 (Cloud Backend Live)
...
├── vite.config.js
├── README.md
├── scripts/                  (data pipeline scripts — not deployed by Vercel)
│   ├── build_political_data.py    (spatial join: venues → local authority → political control)
│   └── build_venue_demographics.py (15-mile catchment demographic profiles)
...
│       └── council_politics.json        (239 KB — 961 venues with political control data)
└── src/
...
- Venue popups on click (name, city, chain, grade, revenue, per-film breakdown for multi-film views)
...
| 6 | React App (live) | https://cine-scope-omega.vercel.app/ | ✅ v2.0.2 |
...
| 28 | Phase 3 Development Plan | CineScope_Phase3_Plan.docx | ✅ Audience intelligence + film research roadmap |
| 29 | Political Data Pipeline | scripts/build_political_data.py | ✅ Spatial join: 961 venues → local authority → party |
| 30 | Council Politics Data | public/data/council_politics.json | ✅ 961 venues, 100% coverage, in app |
...
| **10 Mar 2026** | **Phase 3 planning + political data pipeline** | **Scoped Phase 3 across 3 tracks: Track 1 (demographic catchment profiles), Track 2 (political/council data), Track 3 (TMDB film research). Austin confirmed: TMDB already integrated in a separate session. Political data pipeline built and run: `scripts/build_political_data.py` — downloads ONS LAD May 2024 boundaries (361 LAs), runs geopandas point-in-polygon spatial join for all 971 UK venues (2 coastal venues use nearest-centroid fallback), adds Ireland county-level council control from 2024 Irish local elections. Output: `council_politics.json` (239 KB, 961 venues, 100% coverage). Party breakdown: Labour 351, Conservative 324, NOC 99, Fianna Fáil 84, LD 46, SNP 27, SF 10, other 20. Each venue record includes: la_code, la_name, controlling_party, party_name, party_colour, control_type (majority/minority/coalition/noc), country. Name alias fixes applied: "City of Edinburgh" (not "Edinburgh, City of"), "Na h-Eileanan Siar" (not "Eilean Siar"). `council_politics.json` added to `public/data/` in GitHub. Script stored in `scripts/` at repo root. Phase 3 plan document: CineScope_Phase3_Plan.docx. Next session: wire political data into venue tooltip, map overlay, and AI prompt.** |
...
26. ~~Phase 3 Track 3 (TMDB film research)~~ ✅ **COMPLETE** — integrated in separate session. Auto-research on import, genre/cast/director/certification pulled.
27. **⏳ Phase 3 Track 1 (demographic catchment profiles)** — Data pipeline built (build_venue_demographics.py). venue_demographics.json has 841/1060 venues (E&W + Scotland working). **Three bugs to fix before integration:** (a) Ireland SAPS parser returns 0 zones — GEOGID filter issue (`startswith('A')` failing, possibly CSV quoting or national-level row "IE0" at top); (b) Scotland UV101b age column indexing off — Peterhead showing 85.6% under-25, suggests column header rows misaligning index; (c) NI not yet integrated — 31 venues affected, NISRA DZ-level data needed (not in the bulk ZIPs which are LGD-level only).
28. ~~Phase 3 Track 2 (political data)~~ ✅ **COMPLETE** — council_politics.json (961 venues, 100% coverage) in public/data/. Script: scripts/build_political_data.py.
29. **📋 Wire political data into app** — Three integration points: (1) venue tooltip: party badge + council name + control type; (2) new map overlay mode "Political" (4th option in population toggle, party-coloured choropleth); (3) AI analysis prompt context. **Start here in next session.**
30. **📋 Floating map control bar** — ON/OFF + Population + Politics + Intensity dropdown. Replaces current header population controls. Build after political overlay is wired.
31. **📋 Venue popup demographic panels** — Display catchment age/ethnicity/religion/sex/tenure + council info in venue popups. Depends on Track 1 data bug fixes.
32. **📋 Feed demographics + film research into AI prompts** — Enriched AI context: political catchment, demographic profile, TMDB genre/cast/director all feeding Claude for sharper venue-level insights.
33. **📋 Adjustable grade boundaries UI** — planned refinement
34. **📋 AI report PDF export** — export AI reports directly as standalone PDFs
...
### Council Politics Data (Phase 3 Track 2)
- **File:** `public/data/council_politics.json` (239 KB, 961 venues)
- **Pipeline script:** `scripts/build_political_data.py`
- **Method:** geopandas point-in-polygon spatial join against ONS Local Authority District Boundaries (May 2024, 361 LAs). 2 coastal venues fall outside polygon boundaries — resolved using nearest-centroid fallback. Ireland uses county-level lookup table (no boundary spatial join — address/city string matching).
- **Political data vintage:** UK 2024 local elections + by-election updates to March 2026. Ireland 2024 local elections.
- **Venue record shape:** `{ venue_name, lat, lng, la_code, la_name, controlling_party, party_name, party_colour, control_type, country }`
- **Party codes:** Lab, Con, LD, SNP, PC, Grn, Ref, NOC, SF, DUP, UUP, Asp (Tower Hamlets), Res (Residents)
- **Party colours:** Lab #E4003B, Con #0087DC, LD #FAA61A, SNP #FDF38E, PC #005B54, Grn #00B140, Ref #12B6CF, NOC #888888, SF #326760, DUP #D46A4C
- **Known alias pitfalls:** ONS uses "City of Edinburgh" (not "Edinburgh, City of") and "Na h-Eileanan Siar" (not "Eilean Siar"). Both aliases are now present in the lookup table.
- **Refresh cadence:** Regenerate script after UK local elections (usually May each year) and after major by-elections that change council control.
- **Planned app integration:** (1) Venue tooltip: party colour badge + "Labour majority" label; (2) Map overlay: 4th toggle mode "Political" — party-coloured choropleth (separate to population layer); (3) AI prompt context: party breakdown across venues fed to Claude for political-demographic analysis.
